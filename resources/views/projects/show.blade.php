	...
	<canvas width="400" height="450" id="output2"></canvas>
...
<script type="text/javascript">
	...
	function _processFrame2() {
		...
		if(frstfrm) {
			if(navigator.userAgent.toLowerCase().indexOf('firefox')<0) {
				if(alpha.data[w*4]<50) dy=Math.round(6-(400-w)/50)+1;
			}
	      frstfrm=false;
		} else {
		...
		for ( var i=0; i<image.data.length; i+=4 ) {
			...
			if ( makeTrans ) { image.data[i+3] = 0; }
				...
				output2.putImageData(image, 0, 0, 0, dy, w, h-dy);
		}
	...
	function startProcessing2() {
		timer1 = setInterval(_processFrame2, 100);
	}
